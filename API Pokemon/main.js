...
const renderPokemons = (pokemons = []) => {
    ...
    pokemons.forEach(pokemon => {
        ...
        <article class="pokemon-item">
            ...
            <div>
            <button
                onclick="toggleFavorite('${pokemon.id}', '${pokemon.name}', '${pokemon.image}')" class="star-favorite"
            >
...
const toggleFavorite = async (id, name, image) => {
    ...
    localStorage.setItem('numberFavorites', JSON.stringify(pokemonFavorites.length));
    ...
    const pokemonListFavorites = document.querySelector('#favoritesList')
    if (pokemonListFavorites && pokemonListFavorites.innerHTML !==''){
        renderPokemonsFavorites(pokemonFavorites)
    }
}

const favoritePokemonsState = document.querySelector('#favoritePokemons')

const updateContador = () => {
    let numberSaved = localStorage.getItem('numberFavorites') || 0;
    favoritePokemonsState.textContent = `Favoritos: ${pokemonFavorites.length}`
}
...
nextPageButton.addEventListener('click', async () => {
    ...
    // DONE: validar que no pasemos la última página
...
const renderPokemonsFavorites = (pokemonsFavorites = []) => {
    const pokemonListFavorites = document.querySelector('#favoritesList')

    let elementsFavorites = ''

    // DONE: Necesitamos renderizar el nombre del pokemon en una etiqueta article en el elemento section con id pokemonList

    // DONE: Necesitamos agrega una imagen a cada pokemon

    pokemonsFavorites.forEach(pokemonFavorite => {
        elementsFavorites += `
        <article class="pokemon-item">
            <h2>${pokemonFavorite.name}</h2>
            <img src="${pokemonFavorite.image}" width="80" height="80" />
        </article>
        `
    })

    pokemonListFavorites.innerHTML = elementsFavorites

    const buttonErase = document.querySelector('#buttonErase')

    buttonErase.innerHTML = `<button onclick= "eraseFavorites()" class ="favorite-button">Ocultar</button>`

}

const verFavoriteButton = document.querySelector('#verFavorite')

verFavoriteButton.addEventListener('click',async ()=>{
...
const eraseFavorites = () => {
    ...
    pokemonListFavorites.innerHTML= ''
